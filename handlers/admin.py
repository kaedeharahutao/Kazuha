...
class AdminStates(StatesGroup):
    waiting_admin_numbers = State()
    waiting_admin_name = State()
    waiting_navy_numbers = State()
    waiting_navy_name = State()
...
async def admin_global(message: types.Message, state: FSMContext):
    await state.clear()
    await admin_start(message, state)

@router.message(Command("to_vcf"), F.chat.type == "private")
async def to_vcf_from_admin(message: types.Message, state: FSMContext):
    await state.clear()
...
async def to_txt_from_admin(message: types.Message, state: FSMContext):
    await state.clear()
    from handlers.to_txt import to_txt_start
    await to_txt_start(message, state)

@router.message(Command("start"), F.chat.type == "private")
async def start_from_admin(message: types.Message, state: FSMContext):
    await state.clear()
    from handlers.start import start_handler
    await start_handler(message, state)

@router.message(Command("help"), F.chat.type == "private")
async def help_from_admin(message: types.Message, state: FSMContext):
    await state.clear()
...
async def admin_start(message: types.Message, state: FSMContext):
    ...
    log_user(message)
    bot_msg = "Masukkan nomor admin:"
    await message.answer(bot_msg)
    log_bot(bot_msg)
    await state.set_state(AdminStates.waiting_admin_numbers)

@router.message(AdminStates.waiting_admin_numbers, F.chat.type == "private")
async def admin_receive_admin_numbers(message: types.Message, state: FSMContext):
    ...
    if not numbers:
        bot_msg = "Nomor admin tidak valid. Masukkan ulang nomor admin (pisahkan per baris):"
        await message.answer(bot_msg)
        log_bot(bot_msg)
        return
    await state.update_data(admin_numbers=numbers)
    bot_msg = "Masukkan nama admin:"
    await message.answer(bot_msg)
    log_bot(bot_msg)
    await state.set_state(AdminStates.waiting_admin_name)

@router.message(AdminStates.waiting_admin_name, F.chat.type == "private")
async def admin_receive_admin_name(message: types.Message, state: FSMContext):
    if message.text.strip().startswith("/"):
        await state.clear()
        await router.emit(message)
        return
    ...
    if not admin_name:
        bot_msg = "Nama admin tidak boleh kosong. Masukkan nama admin:"
        await message.answer(bot_msg)
        log_bot(bot_msg)
        return
    await state.update_data(admin_name=admin_name)
    bot_msg = "Masukkan nomor navy:"
    await message.answer(bot_msg)
    log_bot(bot_msg)
    await state.set_state(AdminStates.waiting_navy_numbers)

@router.message(AdminStates.waiting_navy_numbers, F.chat.type == "private")
async def admin_receive_navy_numbers(message: types.Message, state: FSMContext):
    if message.text.strip().startswith("/"):
        await state.clear()
        await router.emit(message)
        return
    ...
    numbers = list(dict.fromkeys(numbers))  # Hapus duplikat, urutan tetap
    if not numbers:
        bot_msg = "Nomor navy tidak valid. Masukkan ulang nomor navy (pisahkan per baris):"
        await message.answer(bot_msg)
        log_bot(bot_msg)
        return
    await state.update_data(navy_numbers=numbers)
    bot_msg = "Masukkan nama navy:"
    await message.answer(bot_msg)
    log_bot(bot_msg)
    await state.set_state(AdminStates.waiting_navy_name)

@router.message(AdminStates.waiting_navy_name, F.chat.type == "private")
async def admin_receive_navy_name(message: types.Message, state: FSMContext):
    if message.text.strip().startswith("/"):
        await state.clear()
        await router.emit(message)
        return
    ...
    log_user(message)
    navy_name = message.text.strip()
    if not navy_name:
        bot_msg = "Nama navy tidak boleh kosong. Masukkan nama navy:"
        await message.answer(bot_msg)
        log_bot(bot_msg)
        return
    await state.update_data(navy_name=navy_name)
    bot_msg = "Masukkan nama file:"
    await message.answer(bot_msg)
    log_bot(bot_msg)
    await state.set_state(AdminStates.waiting_filename)
    
@router.message(AdminStates.waiting_filename, F.chat.type == "private")
async def admin_receive_filename(message: types.Message, state: FSMContext):
    if message.text.strip().startswith("/"):
        await state.clear()
        await router.emit(message)
        return
    ...
    navy_name = data.get("navy_name", "Navy")

    # Gabungkan nomor dan nama, penomoran mulai dari 01 untuk admin dan navy
    contact_names = []
    numbers = []
    for idx, nomor in enumerate(admin_numbers):
        contact_names.append(f"{admin_name} {idx+1:02d}")
        numbers.append(nomor)
    for idx, nomor in enumerate(navy_numbers):
        contact_names.append(f"{navy_name} {idx+1:02d}")
        numbers.append(nomor)
